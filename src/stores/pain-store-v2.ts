...
export const usePainStore = create<PainState>((set, get) => ({
  ...
  // Initialize logs from IndexedDB
  initializeLogs: async () => {
    ...
    try {
      ...
      set({
        error:
          error instanceof Error ? error.message : "Failed to load pain logs",
        isLoading: false,
      });
    }
  },
  ...
  fetchAnalytics: async (period: "week" | "month" | "year") => {
    ...
    try {
      ...
      const activityCounts = new Map<string, { sum: number; count: number }>();
      logs.forEach((log) => {
        if (log.activity) {
