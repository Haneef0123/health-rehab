...
export const useMedicationStore = create<MedicationState>((set, get) => ({
  ...
  fetchMedications: async () => {
    ...
    try {
      const store = await dbManager.getStore<Medication>(STORES.MEDICATIONS);
      if (!store) {
  ...
  fetchTodaySchedule: async () => {
    try {
      ...
      const range = IDBKeyRange.bound(["user-1", today], ["user-1", tomorrow]);
  ...
  logIntake: async (medicationId, scheduledTime, status, notes, skipReason) => {
    set({ isLoading: true, error: null });
  ...
  fetchAdherence: async (medicationId) => {
    ...
    try {
      ...
      set({
        error:
          error instanceof Error ? error.message : "Failed to fetch adherence",
        isLoading: false,
      });
    }
  },
