...
interface DietState {
  ...
  getMissedDietDays: (startDate: Date, endDate: Date) => Promise<Date[]>;
...
export const useDietStore = create<DietState>()(
  persist(
    (set, get) => ({
      ...
      // Initialize store - load recent data from IndexedDB
      initializeStore: async () => {
        ...
        try {
          ...
          // Load last 30 days of water logs
          ...
          dietStartDate.setDate(dietStartDate.getDate() - 7);
          await get().fetchDietLogs(dietStartDate, endDate);
      ...
      addDietLog: async (log) => {
        ...
        try {
          const newLog: DietLog = {
            ...log,
            id: `diet-log-${Date.now()}-${Math.random()
              .toString(36)
              .substr(2, 9)}`,
            createdAt: new Date(),
          };
        ...
        try {
          ...
          if (existingLog) {
            ...
            const updated: WaterLog = {
              ...
              achieved:
                existingLog.milliliters + amount >= existingLog.dailyGoal,
            };

            // Save to database
            const store = await dbManager.getStore<WaterLog>(
              STORES.DIET_ENTRIES
            );
            if (!store) {
              set({ isLoading: false });
      ...
      // Get missed diet days (days without any meal or diet log entries)
      getMissedDietDays: async (startDate, endDate) => {
        try {
          const store = await dbManager.getStore<any>(STORES.DIET_ENTRIES);
          if (!store) return [];
          ...
          const entries = await store.getAllByIndex("userId_date", range);
          ...
          entries.forEach((entry: any) => {
            ...
            datesWithEntries.add(entryDate.toDateString());
          });

          // Find all days in range without entries
          ...
          console.error("Failed to get missed diet days:", error);
          return [];
        }
      },
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        fastingSchedules: state.fastingSchedules,
        dietaryRestrictions: state.dietaryRestrictions,
      }),
      onRehydrateStorage: () => {
        return (state, error) => {
          if (error) {
            console.error("[Diet Store] Hydration error:", error);
          } else if (state) {
            console.log("[Diet Store] Hydrated successfully");
